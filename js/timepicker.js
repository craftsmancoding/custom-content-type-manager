 ...
 *
 * Options:
 *   step: (integer) # of minutes to step the time by
 *   startTime: (string) beginning of the range of acceptable times -- should use separator, e.g. '02:00'
 *   endTime: end of the range of acceptable times
 *   separator: (string) separator string to use between hours and minutes (e.g. ':' or '.')
 *   show24Hours: (boolean) use a 24-hour scheme
 */
...
(function(jQuery){
  ...
  jQuery._timePicker = function(elm, settings) {
    ...
    var $tpDiv = jQuery('<div class="time-picker'+ (settings.show24Hours ? '' : ' time-picker-12hours') +'"></div>');
    var $tpList = jQuery('<ul></ul>');
    ...
    jQuery("li", $tpList).mouseover(function() {
      if (!keyDown) {
        jQuery(selectedSelector, $tpDiv).removeClass(selectedClass);
    ...
    var showPicker = function() {
      ...
      jQuery("li", $tpDiv).removeClass(selectedClass);
      ...
      var $matchedTime = jQuery("li:contains(" + formatTime(roundTime, settings) + ")", $tpDiv);
    ...
    jQuery(elm)[event](function(e) {
      ...
      switch (e.keyCode) {
        case 38: // Up arrow.
          ...
          $selected = jQuery(selectedSelector, $tpList);
          ...
          if (prev) {
            ...
            // Loop to next item.
            $selected.removeClass(selectedClass);
            prev = jQuery("li:last", $tpList).addClass(selectedClass)[0];
        ...
        case 40: // Down arrow, similar in behaviour to up arrow.
          ...
          if (next) {
            ...
            $selected.removeClass(selectedClass);
            next = jQuery("li:first", $tpList).addClass(selectedClass)[0];
            $tpDiv[0].scrollTop = 0;
          }
          return false;
          break;
        case 13: // Enter
          if ($tpDiv.is(":visible")) {
            var sel = jQuery(selectedSelector, $tpList)[0];
